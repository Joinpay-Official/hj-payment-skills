...
| 延迟分账/多次分账 | 单次延迟分账、延迟分账查询、多次分账、完结分账、单笔查询、全部查询 | `/allocFunds` + `altHandle.singleLaterAllocate`、`altHandle.allocateQuery`、`altHandle.manyLaterAllocate`、`altHandle.finishAllocate`、`altHandle.altManyOrderQuery`、`altHandle.altManyTotalQuery` | `altHandle.*` JSON method 分发 | Java/Python/Go/PHP，见 [延迟分账与多次分账示例索引](../../hj-joinpay-many-allocate/references/示例代码/接口索引.md)，覆盖 MD5/RSA 签名 | 暂无工程模板 | `method/version/data/rand_str/sign_type/mch_no/sign` |
...
| 接入延迟分账、多次分账或完结分账 | [延迟分账与多次分账 Skill](../../hj-joinpay-many-allocate/SKILL.md) |
