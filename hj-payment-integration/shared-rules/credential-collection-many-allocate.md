...
本文件仅适用于延迟分账/多次分账 `/allocFunds`。聚合支付 `/tradeRt/*` 和二级商户 `/altFunds` 继续使用各自的收集规则。
...
| `many_allocate_api_scope` | 延迟分账请求 / 延迟分账查询 / 多次分账请求 / 完结分账 / 单笔查询 / 全量查询 | 用于判断是否必须收集 `callback_url` |
...
| 4 | `callback_url` | 请输入异步通知地址；延迟分账请求、多次分账请求和完结分账需要 | 以 `http://` 或 `https://` 开头 |
...
__jp_many_allocate_callback_url    = 延迟分账/多次分账/完结分账回调地址
```
