...
def analyze_refund_result(result):
    ...
    if status == "100":
        ...
        # 退款失败，code为具体错误码
        error_hints = {
            "20090001": "支付成功记录不存在 — 请先确认原订单号是否正确",
            "20090002": "退款金额超限 — 检查退款金额是否超过限制",
            "20090004": "退款记录不存在 — 该退款单号不存在",
            "20090012": "退款订单号已存在 — 更换新单号或确认后重试",
            "20090020": "支付订单剩余可退金额不足 — 用queryRefundInfo查剩余可退金额",
            "20090023": "可用余额不足 — 商户账户余额不足",
            "20090026": "该笔退款涉及风险 — 联系客服处理",
            "10080002": "验证签名失败 — 检查密钥和签名算法",
            "10080015": "退款订单号不正确 — 检查退款订单号格式",
            "10080016": "退款金额不正确 — 检查金额格式(两位小数)",
        }
        hint = error_hints.get(code, "详见响应码完整列表")
        analysis["status_text"] = f"❌ 退款失败 ({code})"
        analysis["suggestion"] = f"{analysis['msg']}\n→ {hint}"
    else:
        analysis["status_text"] = f"❓ 未知状态: {status}"
        analysis["suggestion"] = "状态未知，请联系汇聚客服确认。"
