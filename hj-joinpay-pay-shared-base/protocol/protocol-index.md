...
| 延迟分账/多次分账 JSON 协议 | 延迟分账、多次分账 | `/allocFunds` | JSON body | `sign` | [many-allocate-signing-rules.md](many-allocate-signing-rules.md) |
...
4. 用户提到延迟分账、单次延迟分账、多次分账、完结分账、`altHandle.*`、`singleLaterAllocate`、`allocateQuery` 时，进入延迟分账/多次分账 JSON 协议。
...
| 延迟分账/多次分账 `/allocFunds` | 使用二级商户 `sec_key` 规则，或沿用聚合支付 `hmac` 规则 |
