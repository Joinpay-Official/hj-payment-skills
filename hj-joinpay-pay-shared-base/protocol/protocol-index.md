...
3. 用户提到多次分账、延迟分账、完结分账、`altHandle.*`、`/allocFunds` 时，进入多次分账 JSON 协议。
4. 如果接口路径、公共参数或签名字段不在本表内，先按 [new-product-line-template.md](../../hj-payment-integration/shared-rules/new-product-line-template.md) 新增产品线和协议族，不要复用现有规则。
